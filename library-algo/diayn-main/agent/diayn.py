...
class DIAYNAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, skill_dim, obs_dim_weights, skill_type, 
        ...
        if self.skill_type == 'discrete':
            ...
            # print("INSIDE SKILL TYPE DISCRETE")
        else:
    ...
    def returnQNetworks(self, obs, action, reward, next_obs, skill, not_done, logger, step):
        dist = self.actor(next_obs, skill)
        next_action = dist.rsample()
        log_prob = dist(next_action).sum(-1, keepdim=True)
    ...
    def compute_diversity_reward(self, skill, next_obs):
        ...
        # print(f"The skill before unsqueeze is: {skill}, the shape is: {skill.shape}")

        unsqueezed = skill.unsqueeze(1)
        ...
        # print(f"skills log_prob : {skills_log_prob}, type of {type(skills_log_prob)}")
        ...
        # print(f"Next obs shape before narrow is: {next_obs.shape}")
        # print(next_obs)

        #Since we are using Alex's RLkit , we can filter the first two
        next_obs = torch.narrow(next_obs, 1, 0, 2)
        # print(f"Next obs shape after narrow is: {next_obs.shape}")
        # print(next_obs)
        ...
        # print(f"State Prob logits : {state_prob_logits}, it's shape: {state_prob_logits.shape}")
        log_x_i = torch.sum(state_prob_logits * skill, dim=1, keepdim=True)
        # print(f"log_x_i : {log_x_i}, it's shape: {log_x_i.shape}")
    ...
    def update(self, replay_buffer, logger, step):
        ...
        # not_done and not_done_no_max
        # print(f"DIAYN Values inside the Update: not_done : {not_done}, not_done_no_max : {not_done_no_max}")

        # TODO(Mahi): Figure out the correct reward here.
        # Reward should be log q_phi(z | s_t+1) - log p(z)
        # print(f"The skill DIAYN vanilla in update is : {skill}, the next_obs is : {next_obs}")
        # print(f"The skill shape in DIAYN vanilla in update is : {skill.shape}, the next_obs is : {next_obs.shape}")
